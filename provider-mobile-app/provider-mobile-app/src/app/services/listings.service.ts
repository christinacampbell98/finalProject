...
deleteProperty(propertyId):Promise<any>{
  return new Promise<any>((resolve, reject) => {
    this.http.delete('http://localhost:5000/api/properties/delete',propertyId)
  ...
  updateProperty(edits):Promise<any>{
    return new Promise<any>((resolve, reject) => {
      const headers = new HttpHeaders();
      console.log("New run")
      this.http.patch('http://localhost:5000/api/properties/update',edits, {headers})
      .subscribe((response:any) =>{
  ...
  addProperty(newUser) {
    ...
    this.http.post(environment.BaseURL + '/api/properties/create', newUser, {headers}).subscribe((response: any)=>{
